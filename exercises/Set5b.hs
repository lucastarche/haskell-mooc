...
cull val (Node nodeVal left right) = if nodeVal == val then Empty else Node nodeVal (cull val left) (cull val right)
...
isOrdered (Node val left right) = nodeOrdered (Node val left right) && isOrdered left && isOrdered right

nodeOrdered :: Ord a => Tree a -> Bool
nodeOrdered Empty = True
nodeOrdered (Node val Empty Empty) = True
nodeOrdered (Node val (Node valLeft _ _) Empty) = valLeft <= val
nodeOrdered (Node val Empty (Node valRight _ _)) = val <= valRight
nodeOrdered (Node val (Node valLeft _ _) (Node valRight _ _)) = valLeft <= val && val <= valRight
...
walk (StepL : xs) (Node _ left _) = walk xs left
walk (StepR : xs) (Node _ _ right) = walk xs right
...
set [] _ Empty = Empty
set [] x (Node val left right) = Node x left right
set xs _ Empty = Empty
set (StepL : xs) x (Node val left right) = Node val (set xs x left) right
set (StepR : xs) x (Node val left right) = Node val left (set xs x right)
