...
get :: UpDown -> Int
get (UpDownIncreasing x) = x
get (UpDownDecreasing x) = x

-- tick increases an increasing counter by one or decreases a
-- decreasing counter by one
tick :: UpDown -> UpDown
tick (UpDownIncreasing x) = UpDownIncreasing $ x + 1
tick (UpDownDecreasing x) = UpDownDecreasing $ x - 1

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpDownIncreasing x) = UpDownDecreasing x
toggle (UpDownDecreasing x) = UpDownIncreasing x
...
rgb (Invert color) = map (1.0 -) $ rgb color
rgb (Mix color1 color2) = zipWith (\x y -> (x + y) / 2.0) (rgb color1) (rgb color2)
