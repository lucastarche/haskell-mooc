...
import Data.Char (GeneralCategory (Space), toLower)
import Mooc.Todo
...
instance Eq Name where
  (==) (Name left) (Name right) = a == b
    where
      a = map Data.Char.toLower left
      b = map Data.Char.toLower right
...
class Cycle a where
  ...
  stepMany :: Int -> a -> a
  stepMany 0 k = k
  stepMany n k = step (stepMany (n - 1) k)
