...
prettyPrint n xs = prettyPrint' n xs (1, 1)

prettyPrint' :: Size -> [Coord] -> Coord -> String
prettyPrint' n xs (x, y)
  | x == n && y == n =
    if (x, y) `elem` xs
      then "Q\n"
      else ".\n"
  | y == n =
    if (x, y) `elem` xs
      then 'Q' : '\n' : prettyPrint' n xs (nextRow (x, y))
      else '.' : '\n' : prettyPrint' n xs (nextRow (x, y))
  | otherwise =
    if (x, y) `elem` xs
      then 'Q' : prettyPrint' n xs (nextCol (x, y))
      else '.' : prettyPrint' n xs (nextCol (x, y))
...
danger _ [] = False
danger (x, y) (a : as) = sameRow (x, y) a || sameCol (x, y) a || sameDiag (x, y) a || sameAntidiag (x, y) a || danger (x, y) as
...
prettyPrint2' n xs (x, y)
  | x == n && y == n && (x, y) `elem` xs = "Q\n"
  | x == n && y == n && danger (x, y) xs = "#\n"
  | x == n && y == n = ".\n"
  | y == n && (x, y) `elem` xs = 'Q' : '\n' : prettyPrint2' n xs (nextRow (x, y))
  | y == n && danger (x, y) xs = '#' : '\n' : prettyPrint2' n xs (nextRow (x, y))
  | y == n = '.' : '\n' : prettyPrint2' n xs (nextRow (x, y))
  | (x, y) `elem` xs = 'Q' : prettyPrint2' n xs (nextCol (x, y))
  | danger (x, y) xs = '#' : prettyPrint2' n xs (nextCol (x, y))
  | otherwise = '.' : prettyPrint2' n xs (nextCol (x, y))
...
fixFirst n (a : as)
  | snd a > n = Nothing
  | danger a as = fixFirst n (nextCol a : as)
  | otherwise = Just (a : as)
