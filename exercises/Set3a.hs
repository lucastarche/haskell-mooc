...
firstHalf s = take to_take s
  where
    len = length s
    to_take = len `div` 2 + len `mod` 2
...
capitalize = unwords . map (\s -> toUpper (head s) : tail s) . words
...
joinToLength _ [_] = []
joinToLength k (x : xs) =
  [x ++ y | y <- xs, length x + length y == k]
    ++ [y ++ x | y <- xs, length x + length y == k]
    ++ joinToLength k xs
