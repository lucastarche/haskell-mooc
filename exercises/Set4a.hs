...
distinct [] = True
distinct (x : xs) = foldr (\y b -> x /= y && b) True xs && distinct xs
...
incrementKey n = map increaseIf
  where increaseIf (k, v) = if k == n then (k, v + 1) else (k, v)
...
winner scores player1 player2 = if playerScore player1 < playerScore player2 then player2 else player1
  where
    playerScore player = Map.findWithDefault 0 player scores
...
freqs [] = Map.empty
freqs (x : xs) = Map.alter alterKey x $ freqs xs
    where
        alterKey (Just x) = Just $ x + 1
        alterKey Nothing = Just 1
...
transfer from to amount bank
  ...
  | otherwise = Map.insert from (fromAmount - amount) $ Map.insert to (toAmount + amount) bank
  where
    fromAmount = Map.findWithDefault 0 from bank
...
swap i j arr = arr // [(j, firstElem), (i, secondElem)]
  where
    firstElem = arr ! i
    secondElem = arr ! j
...
maxIndex arr = fst $ foldr cmp (head pairs) pairs 
  where
    pairs = assocs arr
