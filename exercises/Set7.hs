...
data State = Start | Error | Finished | Eggs | JustFlour | JustSugar | FlourAndSugar | Mixed
  deriving (Eq, Show)

step :: State -> Event -> State
step Finished _ = Finished
step Start AddEggs = Eggs
step Eggs AddFlour = JustFlour
step Eggs AddSugar = JustSugar
step JustFlour AddSugar = FlourAndSugar
step JustSugar AddFlour = FlourAndSugar
...
average (x :| xs) = total / len
  where
    len = fromIntegral $ 1 + length xs
    total = x + sum xs
...
instance Operation2 Add2 where
  ...
  showOp (Add2 i j) = show i ++ "+" ++ show j

instance Operation2 Subtract2 where
  compute2 (Subtract2 i j) = i - j
  showOp (Subtract2 i j) = show i ++ "-" ++ show j

instance Operation2 Multiply2 where
  compute2 (Multiply2 i j) = i * j
  showOp (Multiply2 i j) = show i ++ "*" ++ show j

show2 :: (Operation2 a) => a -> String
show2 = showOp
...
passwordAllowed s (And l r) = passwordAllowed s l && passwordAllowed s r
passwordAllowed s (Or l r) = passwordAllowed s l || passwordAllowed s r
passwordAllowed s (ContainsSome chars) = foldr (\c curr -> curr || includes s c) False chars
passwordAllowed s (DoesNotContain chars) = foldr (\c curr -> curr && not (includes s c)) True chars

includes :: String -> Char -> Bool
includes [] c = False
includes (curr : s) c = curr == c || includes s c
...
data Arithmetic = Literal Integer | AdditionOp Arithmetic Arithmetic | ProductOp Arithmetic Arithmetic
  deriving (Show)
...
operation s = if s == "*" then ProductOp else AdditionOp

evaluate :: Arithmetic -> Integer
evaluate (Literal x) = x
evaluate (AdditionOp x y) = evaluate x + evaluate y
evaluate (ProductOp x y) = evaluate x * evaluate y

render :: Arithmetic -> String
render (Literal x) = show x
render (AdditionOp x y) = "(" ++ render x ++ "+" ++ render y ++ ")"
render (ProductOp x y) = "(" ++ render x ++ "*" ++ render y ++ ")"
