...
paintSolid color (Shape shape) (Picture base) = Picture f
  where
    f coord = if shape coord then color else base coord
...
paint (Picture pattern) (Shape shape) (Picture base) = Picture f
  where
    f coord = if shape coord then pattern coord else base coord
...
instance Transform Blur where
  apply _ base = Picture (\coord -> avg (total coord base))
    where
      total (Coord x y) (Picture base) =
        base (Coord x y) <> base (Coord (x - 1) y) <> base (Coord (x + 1) y) <> base (Coord x (y - 1)) <> base (Coord x (y + 1))
      avg (Color r g b) = Color (r `div` 5) (g `div` 5) (b `div` 5)
