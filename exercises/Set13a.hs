...
split s = case elemIndex ' ' s of
  Nothing -> Nothing
  Just x -> Just (take x s, drop (x + 1) s)
...
checkNumber (first, second)
  | hasNum first || hasNum second = Nothing
  | otherwise = Just (first, second)
  where
    hasNum :: String -> Bool
    hasNum s = foldr (\a b -> b || isNumber a) False s
...
winner scores player1 player2 =
  lookup player1 scores ?> \x ->
    lookup player2 scores ?> \y ->
      if x >= y then Just player1 else Just player2
...
safeIndex xs i = if valid then Just (xs !! i) else Nothing
  where
    valid = i >= 0 && i < length xs
...
countAndLog f (x : xs) =
  if f x
    then msg (show x) >> countAndLog f xs >>= \x -> return $ x + 1
...
balance accountName = BankOp $ \(Bank bank) -> case Map.lookup accountName bank of
  Nothing -> (0, Bank bank)
  Just x -> (x, Bank bank)
...
rob from to = balance from +> \x -> withdrawOp from x +> \x -> depositOp to x
...
count x = do
  ...
  case lookup x pairs of
    ...
    Just y -> put $ map (\(a, b) -> if a == x then (a, b + 1) else (a, b)) pairs
